...
{
    RECTANGLE* rect = ( RECTANGLE* ) m_heap.heap_alloc<RECTANGLE>();
    rect->end = true;
...
{
    LAYOUT* tmplayout = ( LAYOUT* ) m_heap.heap_alloc<LAYOUT>();
    tmplayout->type = type;
...
LAYOUT* LayoutTree::create_layout_div( const int id )
{
    ...
    div->css = ( CORE::CSS_PROPERTY* ) m_heap.heap_alloc<CORE::CSS_PROPERTY>();
...
LAYOUT* LayoutTree::create_separator()
{
    ...
    DBTREE::NODE* node = ( DBTREE::NODE* ) m_heap.heap_alloc<DBTREE::NODE>();
