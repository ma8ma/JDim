...
DrawAreaBase::DrawAreaBase( const std::string& url )
    ...
    , m_draw_frame( false )
#if GTKMM_CHECK_VERSION(2,22,0)
    , m_back_frame_top( nullptr )
    , m_back_frame_bottom( nullptr )
#else
    , m_back_frame( NULL )
#endif
...
#if GTKMM_CHECK_VERSION(2,22,0)
    m_backscreen = m_window->create_similar_surface(
        Cairo::Content::CONTENT_COLOR, m_view.get_width(), m_view.get_height() );
#else
    m_backscreen.clear();
...
#if GTKMM_CHECK_VERSION(2,22,0)
    m_back_frame_top = m_window->create_similar_surface(
        Cairo::Content::CONTENT_COLOR, m_view.get_width(), WIDTH_FRAME );
    m_back_frame_bottom = m_window->create_similar_surface(
        Cairo::Content::CONTENT_COLOR, m_view.get_width(), WIDTH_FRAME );
#else
    m_back_frame.clear();
...
#if GTKMM_CHECK_VERSION(2,22,0)
        Cairo::RefPtr< Cairo::Context > scroll_cr = Cairo::Context::create( m_backscreen );
        scroll_cr->rectangle( 0.0, 0.0, width_view, height_view );
        scroll_cr->clip();
#else
        rect_clip.set_y( 0 );
...
#if GTKMM_CHECK_VERSION(2,22,0)
            scroll_cr->push_group();
            scroll_cr->set_source( m_backscreen, 0.0, dy );
            scroll_cr->paint();
            scroll_cr->pop_group_to_source();
            scroll_cr->paint();
#else
...
#if GTKMM_CHECK_VERSION(2,22,0)
            m_cr->save();
            m_cr->rectangle( m_clip_marker.x, m_clip_marker.y,
                             m_clip_marker.width, m_clip_marker.height );
            m_cr->clip();
            m_cr->set_source( m_back_marker, m_clip_marker.x, m_clip_marker.y );
            m_cr->paint();
            m_cr->restore();
#else
...
#if GTKMM_CHECK_VERSION(2,22,0)
            m_cr->save();
            m_cr->rectangle( 0.0, 0.0, width_view, height_view );
            m_cr->clip();
            m_cr->set_source( m_back_frame_top, 0.0, 0.0 );
            m_cr->paint();
            m_cr->set_source( m_back_frame_bottom, 0.0, height_view - WIDTH_FRAME );
            m_cr->paint();
            m_cr->restore();
#else
            // [gtkmm <= 2.8] Gdk::GC::set_clip_rectangle( Gdk::Rectangle& rectangle )
...
#if GTKMM_CHECK_VERSION(2,22,0)
        m_cr->save();
        m_cr->rectangle( 0.0, y_screen, width_view, height_screen );
        m_cr->clip();
        m_cr->set_source( m_backscreen, 0.0, 0.0 );
        m_cr->paint();
        m_cr->restore();
#else
    ...
    switch( layout->type ){
            ...
            if( layout->res_number ){
                ...
                if( m_article->is_bookmarked( layout->res_number ) ){
                    ...
                    if( height > 0 ) {
                        ...
                        Cairo::RefPtr< Cairo::Context > cr =
                            Cairo::Context::create( m_backscreen );
                        cr->rectangle( 1.0, y - ci.pos_y + s_top,
                                       m_pixbuf_bkmk->get_width(), height );
                        cr->clip();
                        Gdk::Cairo::set_source_pixbuf( cr, m_pixbuf_bkmk, 1.0,
                                                       y - ci.pos_y + s_top );
                        cr->paint();
#else
                        m_backscreen->draw_pixbuf(
                            m_gc, m_pixbuf_bkmk, 0, s_top, 1,
                            y - ci.pos_y + s_top, m_pixbuf_bkmk->get_width(),
                            height, Gdk::RGB_DITHER_NONE, 0, 0 );
#endif
                    }
                    y += height_bkmk;
                }
                    ...
                    if( m_article->is_posted( layout->res_number ) ){
                        ...
                        if( height > 0 ) {
                            ...
                            Gdk::Cairo::set_source_pixbuf(
                                cr, m_pixbuf_post, 1.0, y - ci.pos_y + s_top );
                            cr->paint();
#else
                            m_backscreen->draw_pixbuf(
                                m_gc, m_pixbuf_post, 0, s_top, 1,
                                y - ci.pos_y + s_top,
                                m_pixbuf_post->get_width(), height,
                    ...
                    if( m_article->is_refer_posted( layout->res_number ) ){
                        ...
                        if( height > 0 ) {
                            ...
                            Cairo::RefPtr< Cairo::Context > cr =
                                Cairo::Context::create( m_backscreen );
                            cr->rectangle( 1.0, y - ci.pos_y + s_top,
                                           m_pixbuf_refer_post->get_width(),
                                           height );
                            cr->clip();
                            Gdk::Cairo::set_source_pixbuf(
                                cr, m_pixbuf_refer_post, 1.0,
                                y - ci.pos_y + s_top );
                            cr->paint();
#else
                            m_backscreen->draw_pixbuf(
                                m_gc, m_pixbuf_refer_post, 0, s_top, 1,
                                y - ci.pos_y + s_top,
        ...
        case DBTREE::NODE_HR:
            if( layout->rect ){
                ...
                Gdk::Cairo::set_source_color( cr, m_color[ color_text ] );
                cr->set_line_width( 1.0 );
                cr->move_to( x, y );
                cr->line_to( x + layout->rect->width - 1.0, y );
                cr->stroke();
#else
...
#if GTKMM_CHECK_VERSION(2,22,0)
        Cairo::RefPtr< Cairo::Context > cr = Cairo::Context::create( m_back_marker );
        cr->rectangle( 0.0, 0.0, m_clip_marker.width, m_clip_marker.height );
        cr->clip();
        cr->set_source( m_cr->get_target(), -m_clip_marker.x, -m_clip_marker.y );
        cr->paint();
#else
        // [gtkmm <= 2.8] Gdk::GC::set_clip_rectangle( Gdk::Rectangle& rectangle )
        // Gdk::GC::set_clip_rectangle( const Gdk::Rectangle& rectangle )
...
#if GTKMM_CHECK_VERSION(2,22,0)
    const double radius = AUTOSCR_CIRCLE / 2.0;
    m_cr->save();
    m_cr->rectangle( m_clip_marker.x, m_clip_marker.y, m_clip_marker.width, m_clip_marker.height );
    m_cr->clip();
    Gdk::Cairo::set_source_color( m_cr, m_color[ COLOR_MARKER ] );
    m_cr->arc( x_marker + radius, y_marker + radius, radius - 1.0, 0.0, 2.0 * M_PI );
    m_cr->stroke();
    m_cr->restore();
#else
    // [gtkmm <= 2.8] Gdk::GC::set_clip_rectangle( Gdk::Rectangle& rectangle )
...
#if GTKMM_CHECK_VERSION(2,22,0)
        Cairo::RefPtr< Cairo::Surface > surface = m_cr->get_target();
        Cairo::RefPtr< Cairo::Context > cr = Cairo::Context::create( m_back_frame_top );
        cr->rectangle( 0.0, 0.0, width_win, WIDTH_FRAME );
        cr->clip();
        cr->set_source( surface, 0.0, 0.0 );
        cr->paint();

        cr = Cairo::Context::create( m_back_frame_bottom );
        cr->rectangle( 0.0, 0.0, width_win, WIDTH_FRAME );
        cr->clip();
        cr->set_source( surface, 0.0, WIDTH_FRAME - height_win );
        cr->paint();
#else
        // [gtkmm <= 2.8] Gdk::GC::set_clip_rectangle( Gdk::Rectangle& rectangle )
        // Gdk::GC::set_clip_rectangle( const Gdk::Rectangle& rectangle )
...
#if GTKMM_CHECK_VERSION(2,22,0)
    m_cr->save();
    Gdk::Cairo::set_source_color( m_cr, m_color[ COLOR_FRAME ] );
    m_cr->set_line_width( 2.0 );
    m_cr->rectangle( 0.0, 0.0, width_win, height_win );
    m_cr->stroke();
    m_cr->restore();
#else
...
bool DrawAreaBase::draw_one_img_node( LAYOUT* layout, const CLIPINFO& ci )
{
            ...
            if( pixbuf ){
                ...
                if( img->get_mosaic() ){
                    ...
                    if( moswidth && mosheight ){
                        ...
                        Cairo::RefPtr< Cairo::Context > cr =
                            Cairo::Context::create( m_backscreen );
                        cr->rectangle( rect->x + 1.0,
                                       ( rect->y + 1.0 ) - ci.pos_y + s_top,
                                       pixbuf->get_width(), height );
                        cr->clip();
                        Gdk::Cairo::set_source_pixbuf(
                            cr,
                            pixbuf2->scale_simple( pixbuf->get_width(),
                                                   pixbuf->get_height(),
                                                   Gdk::INTERP_NEAREST ),
                            rect->x + 1.0,
                            ( rect->y + 1.0 ) - ci.pos_y + s_top );
                        ...
                        m_backscreen->draw_pixbuf(
                            ...
                            pixbuf2->scale_simple( pixbuf->get_width(),
                                                   ...
                                                   Gdk::INTERP_NEAREST ),
                            0, s_top, rect->x + 1,
                            ( rect->y + 1 ) - ci.pos_y + s_top,
                            pixbuf->get_width(), height, Gdk::RGB_DITHER_NONE,
                            0, 0 );
#endif
                    }
                }
                ...
                // 通常
                    ...
                    cr->rectangle( rect->x + 1.0,
                                   ...
                                   pixbuf->get_width(), height );
                    cr->clip();
                    Gdk::Cairo::set_source_pixbuf( cr, pixbuf, rect->x + 1.0,
                                                   ( rect->y + 1.0 ) - ci.pos_y
                                                       + s_top );
                    cr->paint();
#else
                    m_backscreen->draw_pixbuf(
                        m_gc, pixbuf, 0, s_top, rect->x + 1,
                        ( rect->y + 1 ) - ci.pos_y + s_top, pixbuf->get_width(),
                        height, Gdk::RGB_DITHER_NONE, 0, 0 );
#endif
                }
            }
...
#if GTKMM_CHECK_VERSION(2,22,0)
    {
        ...
        cr->rectangle( rect->x, rect->y - ci.pos_y, rect->width, rect->height );
        cr->stroke();
    }
#else
                                ...
                                const int color, const int color_back, const int byte_from, const int byte_to )
{
    ...
    while( rect ){
        ...
        if( width_line ){
            ...
            text_cr->move_to( x, y );
            pango_cairo_show_layout( text_cr->cobj(), m_pango_layout->gobj() );
...
#if GTKMM_CHECK_VERSION(2,22,0)
                text_cr->move_to( x, y + m_font->ascent );
                pango_cairo_show_glyph_string( text_cr->cobj(), font->gobj(), grl.gobj() );
                if( node->bold ) {
                    text_cr->move_to( x + 1.0, y + m_font->ascent );
                    pango_cairo_show_glyph_string( text_cr->cobj(), font->gobj(), grl.gobj() );
                }
#else
...
#if GTKMM_CHECK_VERSION(2,22,0)
                Gdk::Cairo::set_source_color( text_cr, m_color[ color ] );
                text_cr->set_line_width( 1.0 );
                text_cr->move_to( xx, y + m_font->underline_pos );
                text_cr->line_to( xx + width_line, y + m_font->underline_pos );
                text_cr->stroke();
#else
                m_gc->set_foreground( m_color[ color ] );
...
#if GTKMM_CHECK_VERSION(2,22,0)
    // exposeイベントから抜ける前にm_cr.clear()を呼び出す必要がある
    // ローカル変数やスコープガードなどを使用して安全性を高めるべきか
    m_cr = m_window->create_cairo_context();
#endif
...
#if GTKMM_CHECK_VERSION(2,22,0)
        m_cr->save();
        m_cr->rectangle( x, y, width, height );
        m_cr->clip();
        m_cr->set_source( m_backscreen, x, y );
        m_cr->paint();
        m_cr->restore();
#else
        // [gtkmm <= 2.8] Gdk::GC::set_clip_rectangle( Gdk::Rectangle& rectangle )
...
#if GTKMM_CHECK_VERSION(2,22,0)
        m_cr->save();
        Gdk::Cairo::set_source_color( m_cr, m_color[ get_colorid_back() ] );
        m_cr->fill();
        m_cr->restore();
...
#if GTKMM_CHECK_VERSION(2,22,0)
    m_back_marker = m_window->create_similar_surface( Cairo::Content::CONTENT_COLOR, AUTOSCR_CIRCLE, AUTOSCR_CIRCLE );
#else
