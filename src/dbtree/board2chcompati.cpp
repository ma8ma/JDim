...
ArticleBase* Board2chCompati::append_article( const std::string& datbase, const std::string& id, const bool cached )
{
    ...
    ArticleBase* article = insert( std::make_unique<DBTREE::Article2chCompati>( datbase, id, cached, get_charcode() ) );
...
// レス数(最小)であぼーん(グローバル)
//
int Board2chCompati::get_abone_min_number_global() const
{
...
// レス数(最大)であぼーん(グローバル)
//
int Board2chCompati::get_abone_max_number_global() const
{
