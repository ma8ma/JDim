...
void NodeTreeMachi::init_loading()
{
    ...
    // iconv 初期化
    if( ! m_iconv ) m_iconv = std::make_unique<JDLIB::Iconv>( Encoding::utf8, DBTREE::article_charcode( get_url() ) );
        ...
        if( m_tmp_buffer.empty() ){
            ...
            // タイトル取得
                ...
                if( m_regex->exec( reg_subject, line, offset, icase, newline, usemigemo, wchar ) ){
                    ...
                    m_subject_machi = MISC::Iconv( m_regex->str( 1 ), Encoding::utf8, get_charcode() );
#ifdef _DEBUG
