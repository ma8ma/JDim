...
ArticleBase::ArticleBase( const std::string& datbase, const std::string& id, bool cached, const Encoding charcode )
    : SKELETON::Lockable()
    , m_id( id )
    , m_code( HTTP_INIT )
...
void ArticleBase::read_info()
{
    ...
    std::cout << "subject = " << m_subject << std::endl
              ...
              << "status = " << m_status << std::endl
              << "charcode = " << MISC::charcode_to_cstr( get_charcode() ) << std::endl
