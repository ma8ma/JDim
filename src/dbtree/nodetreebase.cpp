...
const char* NodeTreeBase::add_one_dat_line( const char* datline )
{
    ...
    if( i > 3 ) {
        // EXTDAT情報を取得
        if( header->id_header == 1 ) parse_extattr( section[3].data(), section[3].size() );
...
void NodeTreeBase::parse_date_id( NODE* header, std::string_view str )
{
    ...
    for(;;){
        ...
        while( start_block + lng_block < str.size() && str[ start_block + lng_block ] != ' ' ){
        ...
        if( // ID
            ...
            memcpy( tmplink + sizeof( PROTO_ID ) - 1, str.data() + start_block, lng_id );
        ...
        // BE:
            ...
            memcpy( tmplink + sizeof( PROTO_BE ) - 1, str.data() + start_block + strlen_of_BE, lng_id );
        ...
        // テキスト(日付含む)
            ...
            bool no_date = true;
            for( std::size_t i = start_block; i < start_block + lng_block; ++i )
            ...
            // その他のタグはタグを取り除いて中身だけを見る
                ...
                if( CONFIG::get_use_color_html() ){
                    ...
                    while( pos < pos_end && *pos != '>' ){
                        ...
                        const auto remaining_length{ pos_end - ( pos + 1 ) };

                        if( pre_char && remaining_length >= 5 && ! memcmp( pos + 1, "color", 5 ) ) {
                            pos += 6;
                        }
                        else if( pre_char && remaining_length >= 7 && ! memcmp( pos + 1, "fgcolor", 7 ) ) {
                            pos += 8;
                        }
                        else if( pre_char && remaining_length >= 10 && ! memcmp( pos + 1, "foreground", 10 ) ) {
                            pos += 11;
                        }
                        else if( pre_char && remaining_length >= 7 && ! memcmp( pos + 1, "bgcolor", 7 ) ) {
                            pos += 8;
                            background = true;
                        }
                        else if( pre_char && remaining_length >= 16 && ! memcmp( pos + 1, "background-color", 16 ) ) {
                            pos += 17;
                            background = true;
                        }
                        else if( pre_char && remaining_length >= 10 && ! memcmp( pos + 1, "background", 10 ) ) {
                            pos += 11;
                            background = true;
                        }
