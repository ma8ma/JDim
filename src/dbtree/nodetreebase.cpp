...
std::list< int > NodeTreeBase::get_res_query( const std::string& query, const bool mode_or )
{
    ...
    for( const std::string& qstr : MISC::split_line( query ) ) {
        list_regex.emplace_back( qstr, icase, newline, usemigemo, wchar, norm );
    }
    ...
    for( ; pos < pos_end; ++pos, digitlink = false ){
        ...
        if( *pos == '<' ){ 
            ...
            //  ahref == true かつ <a href=～></a>
                ...
                if( lng_link && lng_str ){
                    ...
                    create_node_link( pos_str_start, lng_str , pos_link_start, lng_link, fgcolor, bgcolor, in_bold );
                }
            }
        ...
        if( linktype != MISC::SCHEME_NONE ){
            ...
            if( linktype == MISC::SCHEME_SSSP ){
                ...
                // 一般リンク
                else create_node_link( tmpstr, lng_str, tmpreplace, lng_replace, COLOR_CHAR_LINK, bgcolor, bold );
            }
                                    ...
                                    const bool abone_board, const bool abone_global )
{
    ...
    for( const std::string& regstr : list_str ) {
        m_list_abone_regex.emplace_back( regstr, icase, newline, false, wchar );
    }
    ...
    for( const std::string& regstr : list_str ) {
        m_list_abone_regex_board.emplace_back( regstr, icase, newline, false, wchar );
    }
    ...
    for( const std::string& regstr : list_str ) {
        m_list_abone_regex_global.emplace_back( regstr, icase, newline, false, wchar );
    }
