...
BoardBase::BoardBase( const std::string& root, const std::string& path_board, const std::string& name )
    ...
    , m_name( name )
    , m_charcode_bak( Encoding::unknown )
{
    clear_load_data();
...
void BoardBase::receive_data( const char* data, size_t size )
{
    ...
    if( ! m_iconv ) m_iconv = std::make_unique<JDLIB::Iconv>( Encoding::utf8, get_charcode() );
    ...
    )
{
    ...
    const std::string query_local = MISC::Iconv( query, get_charcode(), Encoding::utf8 );
