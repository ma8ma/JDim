                                         ...
                                         const int number ) const
{
    ...
    if( cmd_out.find( "$TEXTIU" ) != std::string::npos ){
        ...
        cmd_out = MISC::replace_str( cmd_out, "$TEXTIU", MISC::url_encode_split( texti, Encoding::utf8 ) );
    }
    if( cmd_out.find( "$TEXTIX" ) != std::string::npos ){
        if( ! show_replacetextdiag( texti, "$TEXTIX" ) ) return std::string();
        cmd_out = MISC::replace_str( cmd_out, "$TEXTIX", MISC::url_encode_split( texti, Encoding::eucjp ) );
    }
    if( cmd_out.find( "$TEXTIE" ) != std::string::npos ){
        if( ! show_replacetextdiag( texti, "$TEXTIE" ) ) return std::string();
        cmd_out = MISC::replace_str( cmd_out, "$TEXTIE", MISC::url_encode_split( texti, Encoding::sjis ) );
    }
    ...
    if( cmd_out.find( "$TEXTU" ) != std::string::npos ){
        cmd_out = MISC::replace_str( cmd_out, "$TEXTU", MISC::url_encode_split( texti, Encoding::utf8 ) );
    }
    if( cmd_out.find( "$TEXTX" ) != std::string::npos ){
        cmd_out = MISC::replace_str( cmd_out, "$TEXTX", MISC::url_encode_split( texti, Encoding::eucjp ) );
    }
    if( cmd_out.find( "$TEXTE" ) != std::string::npos ){
        cmd_out = MISC::replace_str( cmd_out, "$TEXTE", MISC::url_encode_split( texti, Encoding::sjis ) );
    }
    ...
    if( cmd_out.find( "$INPUTU" ) != std::string::npos ){
        ...
        cmd_out = MISC::replace_str( cmd_out, "$INPUTU", MISC::url_encode_split( input, Encoding::utf8 ) );
    }
    if( cmd_out.find( "$INPUTX" ) != std::string::npos ){
        if( ! show_replacetextdiag( input, "$INPUTX" ) ) return std::string();
        cmd_out = MISC::replace_str( cmd_out, "$INPUTX", MISC::url_encode_split( input, Encoding::eucjp ) );
    }
    if( cmd_out.find( "$INPUTE" ) != std::string::npos ){
        if( ! show_replacetextdiag( input, "$INPUTE" ) ) return std::string();
        cmd_out = MISC::replace_str( cmd_out, "$INPUTE", MISC::url_encode_split( input, Encoding::sjis ) );
    }
