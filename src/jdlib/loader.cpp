...
std::string Loader::analyze_header_option( std::string_view option ) const
{
    ...
    for( ; ( p1 = strpbrk( p2, accept ) ) != nullptr ; p2 = p1 + 1 ){
        if( strncasecmp( p1, option.data(), option.length() ) == 0 ) break;
    }
...
bool Loader::unzip( char* buf, std::size_t read_size )
{
    ...
    if( m_zstream.avail_in ) {
        std::memmove( m_buf_zlib_in.data(), m_buf_zlib_in.data() + ( m_lng_buf_zlib_in - m_zstream.avail_in ),  m_zstream.avail_in );
    }
