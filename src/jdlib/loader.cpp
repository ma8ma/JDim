...
int Loader::check_gzheader( char* buf, size_t& size )
{
    ...
    // FLG(1byte)
    const unsigned char flag = ( ( unsigned char* )buf )[ pos++ ];
#ifdef _DEBUG
    std::cout << "flag = " << std::hex << static_cast< unsigned int >( flag ) << std::dec << std::endl;
#endif
...
bool Loader::unzip( char* buf, size_t& read_size )
{
    ...
    // gzip のヘッダを飛ばす
    if( m_check_gzheader ){
        const int headsize = check_gzheader( buf, read_size );
        if( headsize < 0 ){
