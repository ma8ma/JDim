    ...
    , m_coding_to( to )
{
    ...
    const char* to_str = MISC::charcode_to_cstr( ( to == Encoding::unknown ) ? from : to );

    m_cd = g_iconv_open( to_str, from_str );

    // MS932で失敗したらCP932で試してみる
    if( m_cd == ( GIConv ) -1 ){
        if( to == Encoding::sjis ) m_cd = g_iconv_open( "CP932", from_str );
        else if( from == Encoding::sjis ) m_cd = g_iconv_open( to_str, "CP932" );
    }

    // "EUCJP-*"で失敗したら"EUCJP"で試してみる
    if( m_cd == ( GIConv ) - 1 && ( errno & EINVAL ) != 0 )
    {
        if( to == Encoding::eucjp ) m_cd = g_iconv_open( "EUCJP//TRANSLIT", from_str );
        else if( from == Encoding::eucjp )
        {
    ...
    do{
        ...
        if( ret == -1 ){
            ...
            if( errno == EILSEQ ){
                ...
                if( m_coding_from == Encoding::sjis && m_coding_to == Encoding::utf8
                ...
                // unicode 文字からの変換失敗
                // 数値文字参照(&#????;)形式にする
                if( m_coding_from == Encoding::utf8 ){
