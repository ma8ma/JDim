...
namespace MISC
{
    ...
    // urlエンコード
    std::string url_encode( const char* str, const size_t n, const Encoding charcode = Encoding::utf8 );
    inline std::string url_encode( const std::string& str, const Encoding charcode = Encoding::utf8 )
    {
        return url_encode( str.c_str(), str.length(), charcode );
    }

    // 半角スペースまたは "" 単位で区切って url エンコード
    std::string url_encode_split( const std::string& str, const Encoding charcode );
