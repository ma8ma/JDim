...
bool Socket::tls_handshake( const std::string& hostname, const bool verify )
{
    ...
    if( ! SSL_CTX_set_min_proto_version( m_ctx, TLS1_1_VERSION ) ) {
        m_errmsg = "SSL_CTX_set_min_proto_version failed: ";
        m_errmsg += ssllib_errstr();
        goto EXIT_TLS_CONNECT;
    }
#ifdef SSL_OP_IGNORE_UNEXPECTED_EOF
    // skip chunked を正確に実装するまではエラーの無視が必要
    SSL_CTX_set_options( m_ctx, SSL_OP_IGNORE_UNEXPECTED_EOF );
#endif

    if( verify ){
        // load the trusted client CA certificate into context
        if( SSL_CTX_set_default_verify_paths( m_ctx ) != 1 ){
            m_errmsg = "SSL_CTX_set_default_verify_paths failed: ";
            m_errmsg += ssllib_errstr();
            goto EXIT_TLS_CONNECT;
        }
