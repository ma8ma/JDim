...
void FontColorPref::pack_widget()
{
    ...
    constexpr const char* ss_tip =
        "スレビューにおいて行の高さを調節します( 標準は 1 )";
#if GTKMM_CHECK_VERSION(2,12,0)
    ...
    constexpr const char* su_tip =
        "スレビューにおいてアンカーなどの下線の位置を調節します( 標準は 1 )";
#if GTKMM_CHECK_VERSION(2,12,0)
...
void FontColorPref::slot_change_color()
{
    ...
    std::vector< Gtk::TreePath > selection_path =
        m_treeview_color.get_selection()->get_selected_rows();
...
#endif
                CONFIG::set_color(
                    colorid, MISC::color_to_str( sel->get_current_color() ) );
            }
        }
    }
}
