...
TEST_F(Iconv_ToAsciiFromUtf8, empty)
{
    ...
    JDLIB::Iconv icv( CHARCODE_ASCII, CHARCODE_UTF8 );
...
TEST_F(Iconv_ToUtf8FromMs932, empty)
{
    ...
    JDLIB::Iconv icv( CHARCODE_UTF8, CHARCODE_SJIS );
    const char* result = icv.convert( input, std::strlen(input), size_out );
...
// skip due to SIGSEGV
TEST_F(Iconv_ToUtf8FromMs932, DISABLED_hex_a0)
{
...
// skip due to SIGSEGV
TEST_F(Iconv_ToUtf8FromMs932, DISABLED_mojibake_fix_inequality_sign_pattern1)
{
...
// skip due to SIGSEGV
TEST_F(Iconv_ToUtf8FromMs932, DISABLED_mojibake_fix_inequality_sign_pattern2)
{
    // DATのデータ区切り <> が文字化けするとスレが壊れるため変換を修正する
    // エンコーディングがMS932のスレにUTF-8で書き込み文字化けした場合をテスト
    char input[] = "<> test テスト <>";
...
// skip due to SIGSEGV
TEST_F(Iconv_ToUtf8FromMs932, DISABLED_mapping_error)
{
